#!/usr/bin/env python
# VERSION: 1.0-MMD
# ARCHITECTURE: Parallel intra-cycle CNN/GRU + inter-cycle GRU + age-matched MK-MMD
# FILE: hierarchical_hybrid_cnn_gru_mmd_rul_lodo_v1.py
# PATH POLICY:
...
"""
BatteryLife multi-source unsupervised domain adaptation for RUL
================================================================
...
Transfer:
    L_total = L_RUL(source labels)
              + lambda_MMD * L_MMD(source embedding, target embedding)

    MMD is calculated between source and unlabeled target battery embeddings
    observed at the same absolute cycle count.

Evaluation:
    Leave-One-Dataset-Out (LODO)
    - Target RUL/life labels are never used during training.
    - Unlabeled target curves are used only by the MMD adaptation loss.
    - Target life labels are opened only after training for evaluation.
    - Evaluation uses multiple life-fraction and absolute-cycle observation points.
...
Expected project structure:
    ...
    │     └─ Tongji_labels.json
    ├─ model/
    │  └─ hierarchical_hybrid_cnn_gru_mmd_rul_lodo_v1.py
    └─ run/
       └─ hierarchical_hybrid_cnn_gru_mmd_target_HUST_YYYYMMDD_HHMMSS/

Example:
    python model/hierarchical_hybrid_cnn_gru_mmd_rul_lodo_v1.py --target HUST --model-name hierarchical_hybrid_cnn_gru_mmd

Useful quick test:
    python model/hierarchical_hybrid_cnn_gru_mmd_rul_lodo_v1.py --target HUST \
...
Notes:
    ...
    9. Source and target embeddings are aligned only at matched cycle ages.
    10. Target labels are inaccessible until the final evaluation stage.
    11. Interpolated cycle tensors are cached as float16 .npy files.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "BatteryLife LODO Model D: hierarchical parallel CNN/GRU "
            "with age-matched multi-kernel MMD adaptation."
        ),
    ...
    parser.add_argument(
        "--model-name",
        type=str,
        default="hierarchical_hybrid_cnn_gru_mmd",
    ...
    parser.add_argument(
        ...
        default=[20, 50, 100, 200, 500],
        help=(
            "Absolute cycle counts at which source and unlabeled target "
            "battery embeddings are aligned."
        ),
    )
    parser.add_argument(
        "--mmd-weight",
        type=float,
        default=0.10,
        help="Maximum weight of the age-matched multi-kernel MMD loss.",
    )
    parser.add_argument(
        "--mmd-ramp-epochs",
        type=int,
        default=5,
        help=(
            "Linearly increase the MMD weight during these initial epochs. "
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Batch size used separately for source and target MMD features.",
    )
    parser.add_argument(
        "--mmd-kernel-multipliers",
        type=float,
        nargs="+",
        default=[0.25, 0.5, 1.0, 2.0, 4.0],
        help=(
            "RBF bandwidth multipliers used by the multi-kernel MMD. "
            "The base bandwidth is selected with a detached median heuristic."
        ),
    )
    parser.add_argument(
        "--disable-mmd-feature-normalization",
        action="store_true",
        help="Disable L2 normalization of battery embeddings before MMD.",
    )
    parser.add_argument(
        "--max-sequence-cycles",
        type=int,
...
def validate_benchmark_arguments(args: argparse.Namespace) -> None:
    ...
    if not args.adapt_cycle_counts:
        raise ValueError("--adapt-cycle-counts must not be empty.")
    if any(value < args.min_prefix_cycles for value in args.adapt_cycle_counts):
        raise ValueError(
            "Every --adapt-cycle-counts value must be at least "
            "--min-prefix-cycles."
        )
    if args.mmd_weight < 0:
        raise ValueError("--mmd-weight must be non-negative.")
    if args.mmd_ramp_epochs < 0:
        raise ValueError("--mmd-ramp-epochs must be >= 0.")
    if args.alignment_batch_size < 2:
        raise ValueError("--alignment-batch-size must be at least 2.")
    if not args.mmd_kernel_multipliers:
        raise ValueError("--mmd-kernel-multipliers must not be empty.")
    if any(value <= 0 for value in args.mmd_kernel_multipliers):
        raise ValueError(
            "Every --mmd-kernel-multipliers value must be positive."
        )
...
def build_age_matched_alignment_groups(
    records: Sequence[BatteryRecord],
    cache_map: Dict[Path, Tuple[Path, Path]],
    min_prefix_cycles: int,
    alignment_cycle_counts: Sequence[int],
    source_has_life_labels: bool,
) -> Dict[int, List[AdaptationPrefixSample]]:
    """
    Build prefix groups for age-conditioned source-target alignment.

    Source:
        The life label is used only to reject prefixes at/after EOL.

    Target:
        No life label is accessed. Eligibility is based only on the number
        of currently available cycles.
    """
    groups: Dict[int, List[AdaptationPrefixSample]] = {
        int(cycle_count): []
        for cycle_count in sorted(set(alignment_cycle_counts))
    }
    ...
    for record in records:
        ...
        _, cycle_numbers = load_cached_battery(cache_pair)
        if len(cycle_numbers) < min_prefix_cycles:
            continue

        if source_has_life_labels:
            if record.life <= 0:
                raise ValueError(
                    "A source alignment record is missing its life label: "
                    f"{record.file_name}"
                )
            valid_indices = np.flatnonzero(cycle_numbers < record.life)
            if valid_indices.size < min_prefix_cycles:
            ...
            # Strictly unlabeled target path.
            max_valid_length = int(len(cycle_numbers))

        for alignment_cycle in groups:
            prefix_length = prefix_length_for_cycle_age(
                cycle_numbers=cycle_numbers,
                desired_cycle_age=alignment_cycle,
                min_prefix_cycles=min_prefix_cycles,
                max_valid_length=max_valid_length,
            )
            ...
            current_cycle = int(cycle_numbers[prefix_length - 1])
            if current_cycle < alignment_cycle:
                continue
            if source_has_life_labels and current_cycle >= record.life:
                continue

            groups[alignment_cycle].append(
                AdaptationPrefixSample(
                    record=record,
                    prefix_length=prefix_length,
                    current_cycle=current_cycle,
...
class AdaptationPrefixDataset(Dataset):
    """Prefix dataset without RUL labels for MMD feature alignment."""

    def __init__(
        self,
        samples: Sequence[AdaptationPrefixSample],
        cache_map: Dict[Path, Tuple[Path, Path]],
        scaler: ChannelScaler,
        max_sequence_cycles: int,
    ) -> None:
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        return {
            ...
            "dataset": sample.record.dataset,
            "file_name": sample.record.file_name,
            "alignment_cycle": int(sample.alignment_cycle),
        }


def collate_adaptation_batch(batch: Sequence[dict]) -> dict:
    ...
    for i, item in enumerate(batch):
        length = item["length"]
        x[i, :length] = item["x"]
        cycle_ids[i, :length] = item["cycle_ids"]
    ...
    return {
        ...
        "alignment_cycle": int(batch[0]["alignment_cycle"]),
    }
...
class BatteryRULModel(nn.Module):
    ...
    def forward(
        ...
        battery_embedding = self.encode_battery(x, lengths)
        predicted_log_rul = self.rul_head(battery_embedding).squeeze(-1)

        if return_embedding:
            return predicted_log_rul, battery_embedding
...
def pairwise_squared_distance(
    ...
    y_norm = torch.sum(y * y, dim=1, keepdim=True).transpose(0, 1)
    distance = x_norm + y_norm - 2.0 * x @ y.transpose(0, 1)
...
def multi_kernel_mmd(
    ...
    kernel_multipliers: Sequence[float],
    normalize_features: bool,
) -> torch.Tensor:
    """
    Stable biased MK-MMD estimator.

    The biased estimator includes diagonal kernel terms. It is non-negative
    up to floating-point error and is more stable than the unbiased estimator
    for the small alignment batches used by this baseline.
    """
    if source_features.ndim != 2 or target_features.ndim != 2:
        raise ValueError("MMD features must be two-dimensional [B, D].")
    if source_features.shape[1] != target_features.shape[1]:
        raise ValueError("Source and target MMD feature dimensions must match.")
    ...
    if normalize_features:
        source = F.normalize(source, p=2, dim=1, eps=1e-8)
        target = F.normalize(target, p=2, dim=1, eps=1e-8)

    combined = torch.cat([source, target], dim=0)
    combined_distance = pairwise_squared_distance(combined, combined)

    with torch.no_grad():
        off_diagonal_mask = ~torch.eye(
            combined_distance.shape[0],
            dtype=torch.bool,
            device=combined_distance.device,
        )
        positive = combined_distance[off_diagonal_mask]
        positive = positive[positive > 1e-12]
        if positive.numel() == 0:
            base_bandwidth = combined_distance.new_tensor(1.0)
        else:
            base_bandwidth = torch.median(positive).clamp_min(1e-6)

    source_distance = pairwise_squared_distance(source, source)
    target_distance = pairwise_squared_distance(target, target)
    cross_distance = pairwise_squared_distance(source, target)

    kernels_xx = source.new_zeros(source_distance.shape)
    kernels_yy = target.new_zeros(target_distance.shape)
    kernels_xy = source.new_zeros(cross_distance.shape)

    for multiplier in kernel_multipliers:
        bandwidth = base_bandwidth * float(multiplier)
        kernels_xx = kernels_xx + torch.exp(
            -source_distance / bandwidth.clamp_min(1e-6)
        )
        kernels_yy = kernels_yy + torch.exp(
            -target_distance / bandwidth.clamp_min(1e-6)
        )
        kernels_xy = kernels_xy + torch.exp(
            -cross_distance / bandwidth.clamp_min(1e-6)
        )

    num_kernels = float(len(kernel_multipliers))
    kernels_xx = kernels_xx / num_kernels
    kernels_yy = kernels_yy / num_kernels
    kernels_xy = kernels_xy / num_kernels

    mmd = kernels_xx.mean() + kernels_yy.mean() - 2.0 * kernels_xy.mean()
    return mmd.clamp_min(0.0)


def mmd_weight_at_epoch(
...
def train_one_epoch(
    ...
    source_alignment_loaders: Dict[int, DataLoader],
    target_alignment_loaders: Dict[int, DataLoader],
    optimizer: torch.optim.Optimizer,
    device: torch.device,
    grad_clip: float,
    current_mmd_weight: float,
    mmd_kernel_multipliers: Sequence[float],
    normalize_mmd_features: bool,
) -> Dict[str, float]:
    model.train()

    common_alignment_cycles = sorted(
        set(source_alignment_loaders) & set(target_alignment_loaders)
    )
    if current_mmd_weight > 0 and not common_alignment_cycles:
        raise RuntimeError(
            "MMD is enabled, but no source-target alignment cycle is available."
        )

    source_iterators = {
        cycle: iter(loader)
        for cycle, loader in source_alignment_loaders.items()
    }
    target_iterators = {
        cycle: iter(loader)
        for cycle, loader in target_alignment_loaders.items()
    }
    ...
    for step, batch in enumerate(supervised_loader):
        batch = move_batch_to_device(batch, device)
        optimizer.zero_grad(set_to_none=True)

        pred_log_rul, _ = model(
            batch["x"],
            batch["lengths"],
        ...
        mmd_loss = regression_loss.new_zeros(())

        if current_mmd_weight > 0 and common_alignment_cycles:
            alignment_cycle = common_alignment_cycles[
                step % len(common_alignment_cycles)
            ]

            source_alignment_batch, source_iterators[alignment_cycle] = (
                next_or_restart(
                    source_iterators[alignment_cycle],
                    source_alignment_loaders[alignment_cycle],
                )
            )
            target_alignment_batch, target_iterators[alignment_cycle] = (
                next_or_restart(
                    target_iterators[alignment_cycle],
                    target_alignment_loaders[alignment_cycle],
                )
            )

            source_alignment_batch = move_batch_to_device(
                source_alignment_batch,
                device,
            )
            target_alignment_batch = move_batch_to_device(
                target_alignment_batch,
                device,
            )

            source_embedding = model.encode_battery(
                source_alignment_batch["x"],
                source_alignment_batch["lengths"],
            )
            target_embedding = model.encode_battery(
                target_alignment_batch["x"],
                target_alignment_batch["lengths"],
            )

            mmd_loss = multi_kernel_mmd(
                source_features=source_embedding,
                target_features=target_embedding,
                kernel_multipliers=mmd_kernel_multipliers,
                normalize_features=normalize_mmd_features,
            )
            mmd_steps += 1

        loss = regression_loss + current_mmd_weight * mmd_loss

        if not torch.isfinite(loss):
            raise FloatingPointError(
                "Non-finite training loss: "
                f"total={loss.item()}, "
                f"regression={regression_loss.item()}, "
                f"mmd={mmd_loss.item()}"
            )
        ...
        mmd_loss_sum += float(mmd_loss.item())
        total_count += batch_size

    return {
        "total_loss": total_loss_sum / max(total_count, 1),
        "regression_loss": regression_loss_sum / max(total_count, 1),
        "mmd_loss": mmd_loss_sum / max(mmd_steps, 1),
        "mmd_weight": float(current_mmd_weight),
        "mmd_steps": float(mmd_steps),
    }
...
def create_adaptation_loader(
    ...
    if len(dataset) < 2:
        raise ValueError(
            "An adaptation dataset needs at least two batteries for MMD."
        )

    effective_batch_size = min(requested_batch_size, len(dataset))
    if effective_batch_size < 2:
        raise ValueError("Effective MMD batch size must be at least two.")

    # Drop only an undersized remainder so every returned MMD batch has
    # a stable sample count of at least two.
    ...
    return DataLoader(
        ...
        shuffle=True,
        num_workers=num_workers,
        collate_fn=collate_adaptation_batch,
        pin_memory=(device.type == "cuda"),
        persistent_workers=(num_workers > 0),
        drop_last=drop_last,
    )
...
def main() -> None:
    ...
    if args.project_root is None:
        # Expected script location: project/model/hierarchical_hybrid_cnn_gru_mmd_rul_lodo_v1.py
    ...
    print(
        ...
        "-> inter-cycle GRU -> age-matched MK-MMD"
    )
    print(
        "dimensions   : "
    ...
    source_alignment_groups = build_age_matched_alignment_groups(
        records=train_records,
        cache_map=cache_map,
        min_prefix_cycles=args.min_prefix_cycles,
        alignment_cycle_counts=args.adapt_cycle_counts,
        source_has_life_labels=True,
    )
    target_alignment_groups = build_age_matched_alignment_groups(
        records=target_records,
        cache_map=cache_map,
        min_prefix_cycles=args.min_prefix_cycles,
        alignment_cycle_counts=args.adapt_cycle_counts,
        source_has_life_labels=False,
    )

    common_alignment_cycles = [
        cycle
        for cycle in sorted(set(source_alignment_groups) & set(target_alignment_groups))
        if len(source_alignment_groups[cycle]) >= 2
        and len(target_alignment_groups[cycle]) >= 2
    ]

    if args.mmd_weight > 0 and not common_alignment_cycles:
        raise RuntimeError(
            "MMD adaptation cannot start because no requested cycle count "
            "has at least two source and two unlabeled target batteries."
        )

    source_alignment_groups = {
        cycle: source_alignment_groups[cycle]
        for cycle in common_alignment_cycles
    }
    target_alignment_groups = {
        cycle: target_alignment_groups[cycle]
        for cycle in common_alignment_cycles
    }
    ...
    print(
        ...
        "Target curves are used without labels only for MMD alignment."
    )
    print(
        "[adaptation policy] Age-matched battery-embedding MK-MMD at cycles: "
        + ", ".join(str(value) for value in common_alignment_cycles)
    )
    for cycle in common_alignment_cycles:
        print(
            f"[alignment coverage] cycle={cycle}: "
            f"source={len(source_alignment_groups[cycle])}, "
            f"target={len(target_alignment_groups[cycle])}"
        )
    ...
    for cycle in common_alignment_cycles:
        source_alignment_dataset = AdaptationPrefixDataset(
            samples=source_alignment_groups[cycle],
            cache_map=cache_map,
            scaler=scaler,
            max_sequence_cycles=args.max_sequence_cycles,
        )
        target_alignment_dataset = AdaptationPrefixDataset(
            samples=target_alignment_groups[cycle],
            cache_map=cache_map,
            scaler=scaler,
            max_sequence_cycles=args.max_sequence_cycles,
        )

        source_alignment_loaders[cycle] = create_adaptation_loader(
            dataset=source_alignment_dataset,
            requested_batch_size=args.alignment_batch_size,
            num_workers=args.num_workers,
            device=device,
        )
        target_alignment_loaders[cycle] = create_adaptation_loader(
            dataset=target_alignment_dataset,
            requested_batch_size=args.alignment_batch_size,
            num_workers=args.num_workers,
            device=device,
        )
        ...
        json.dump(
            {
                ...
                "architecture": {
                    "name": (
                        "hierarchical_parallel_cnn_gru_then_inter_gru"
                        "_with_age_matched_mk_mmd"
                    ),
                ...
                "adaptation": {
                    "type": "unsupervised_domain_adaptation",
                    "feature_level": "final_battery_embedding",
                    "loss": "age_matched_multi_kernel_mmd",
                    "alignment_cycle_counts_requested": args.adapt_cycle_counts,
                    "alignment_cycle_counts_used": common_alignment_cycles,
                    "source_samples_by_cycle": {
                        str(cycle): len(source_alignment_groups[cycle])
                        for cycle in common_alignment_cycles
                    },
                    "target_unlabeled_samples_by_cycle": {
                        str(cycle): len(target_alignment_groups[cycle])
                        for cycle in common_alignment_cycles
                    },
                    "maximum_mmd_weight": args.mmd_weight,
                    "mmd_ramp_epochs": args.mmd_ramp_epochs,
                    "alignment_batch_size": args.alignment_batch_size,
                    "kernel_multipliers": args.mmd_kernel_multipliers,
                    "feature_l2_normalization": (
                        not args.disable_mmd_feature_normalization
                    ),
    ...
    for epoch in range(1, args.epochs + 1):
        current_mmd_weight = mmd_weight_at_epoch(
            maximum_weight=args.mmd_weight,
            epoch=epoch,
            ramp_epochs=args.mmd_ramp_epochs,
        )
        train_metrics = train_one_epoch(
            model=model,
            supervised_loader=train_loader,
            source_alignment_loaders=source_alignment_loaders,
            target_alignment_loaders=target_alignment_loaders,
            optimizer=optimizer,
            device=device,
            grad_clip=args.grad_clip,
            current_mmd_weight=current_mmd_weight,
            mmd_kernel_multipliers=args.mmd_kernel_multipliers,
        ...
        print(
            f"[epoch {epoch:03d}] "
            f"total={train_metrics['total_loss']:.5f} | "
            f"reg={train_metrics['regression_loss']:.5f} | "
            f"mmd={train_metrics['mmd_loss']:.5f} | "
            f"lambda={train_metrics['mmd_weight']:.4f} | "
            f"val={val_loss:.5f} | "
