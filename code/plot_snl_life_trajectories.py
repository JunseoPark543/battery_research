#!/usr/bin/env python3
"""Draw all SNL battery life trajectories together in one plot."""
...
DEFAULT_DATA_DIR = PROJECT_ROOT / "data" / "SNL"
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "plot" / "SNL" / "life_trajectories"
...
def main() -> None:
    ...
    if not trajectories:
        raise RuntimeError("No valid SNL trajectories were found.")
    ...
    threshold = axis.axhline(
        80.0, color="black", linestyle="--", linewidth=1.2, label="80% SOH"
    )
    axis.set(
        title=f"SNL Battery Life Trajectories ({len(trajectories)} cells)",
        xlabel="Cycle number",
        ylabel="SOH relative to cycle 1 (%)",
    )
    axis.grid(alpha=0.25)
    axis.legend(handles=[threshold], loc="lower left")
    fig.tight_layout()

    output_path = args.output_dir / "SNL_all_life_trajectories.png"
