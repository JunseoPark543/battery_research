#!/usr/bin/env python3
"""Draw all CALCE battery life trajectories together in one plot."""
...
DEFAULT_DATA_DIR = PROJECT_ROOT / "data" / "CALCE"
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "plot" / "CALCE" / "life_trajectories"
...
def main() -> None:
    ...
    if not trajectories:
        raise RuntimeError("No valid CALCE trajectories were found.")
    ...
    axis.set(
        title=f"CALCE Battery Life Trajectories ({len(trajectories)} cells)",
        xlabel="Cycle number",
    ...
    output_path = args.output_dir / "CALCE_all_life_trajectories.png"
